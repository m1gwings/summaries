...
17. If an ARMA process is stationary, we can exploit stationarity to compute its expected value from the expression which defines the process. To compute the covariance function it is possible to generalize the Yule-Walker equations to ARMA processes (_see property 15_).
In particular, for a process:
$$
v_1(t) = \frac{C(z)}{A(z)} v_2(t)
$$
> it holds that:
$$
\mu_{v_1} = \frac{C(1)}{A(1)} \mu_{v_2} = W(1) \mu_{v_2}.
$$

> **Proof**: the expression which defines the process in the time domain is:
$$
v_1(t) = a_1 v_1(t-1) + \ldots + a_{n_a} v_1(t-n_a) + c_0 v_2(t) + \ldots + c_{n_c} v_2(t-n_c).
$$
> By assumption both $v_1$, and $v_2$ are stationary, hence if we take expectation of the expression above, we get:
$$
\mu_{v_1} - a_1 \mu_{v_1} - \ldots - a_{n_a} \mu_{v_{n_a}} = c_0 \mu_{v_2} + \ldots + c_{n_c} \mu_{v_2} \text{ iff }
$$
$$
\mu_{v_1} = \frac{c_0 + \ldots + c_n \cdot 1}{1 - a_1 \cdot 1 - \ldots - a_n \cdot 1} \mu_{v_2} = \frac{C(1)}{A(1)} \mu_{v_2} .
$$

18. Consider an ARMA process defined as:
$$
v_1(t) = \frac{C(z)}{A(z)} v_2(t).
$$
> Then:
$$
\tilde{v}_1(t) = \frac{C(z)}{A(z)} \tilde{v}_2(t)
$$
> where $\tilde{v}_1(t) = v_1(t) - \mu_{v_1}$, $\tilde{v}_2(t) = v_2(t) - \mu_{v_2}$. That is, we can **de-bias the process expression**.

---

> **Proof**: observe that:
$$
c_0 \tilde{v}_2(t) + \ldots + c_{n_c} \tilde{v}_2(t - n_c) = c_0 v_2(t) + \ldots + c_{n_c} v_2(t - n_c) - (c_0 + \ldots + c_{n_c}) \mu_{v_2} =
$$

$$
\stackrel{\text{property } 18}{=} c_0 v_2(t) + \ldots + c_{n_c} v_2(t - n_c) - (1 - a_1 - \ldots - a_{n_a}) \mu_{v_1}  =
$$

$$
= v_1(t) - a_1 v_1(t-1) - \ldots - a_{n_a} v_1(t-n_a) -
$$

$$
- (1 - a_1 - \ldots - a_{n_a}) \mu_{v_1} = \tilde{v}_1(t) - a_1 \tilde{v}_1(t) - \ldots - a_{n_a} \tilde{v}_1(t).
$$

> In operatorial notation this becomes:
$$
C(z) \tilde{v}_2(t) = A(z) \tilde{v}_1(t) \text{ iff}
$$
...
20. The sum of the Fourier series exists only for stationary processes whose correlation function $\tilde{\gamma}_{vv}(\tau)$ tends to $0$ sufficiently rapidly when $\tau$ tends to infinity. A sufficient condition for the existence of the Fourier transform is the absolute convergence of $\tilde{\gamma}_{vv}(\tau)$.

21. Let $\Gamma_{vv}(\omega)$ be the spectrum of a stationary stochastic process $v$. The anti-transformation formula allows to retrieve the correlation function of the process and to establish a one-to-one correspondence between it and the spectrum:
...
22. From the definition of spectrum, remembering that $\tilde{\gamma}$ is even, it follows that:
$$
...
23. For a stationary stochastic processes with zero expected value, the variance of the process is proportional to the are below the spectrum curve.
...
24. For a stationary stochastic process $v$ it holds that:
$$
...
25. The spectrum of $\eta(\cdot) \sim WN(0, \lambda^2)$ is:
$$
...
26. **Fundamental theorem of spectral analysis**: consider the process $y(\cdot)$ obtained by filtering a <u>stationary</u> input process $u(\cdot)$ through an <u>asymptotically stable</u> dynamical system described by the transfer function $W(z)$:
$$
y(t) = W(z) u(t) \text{,}
$$
...
27. Let's list the expressions of the spectrum for some deterministic processes.
...
28. Let's list some **equivalent pairs $(W(z), \eta(\cdot))$**.
...
30. **Spectral factorization theorem**: let $v(\cdot)$ be a stationary stochastic process with rational spectrum.
